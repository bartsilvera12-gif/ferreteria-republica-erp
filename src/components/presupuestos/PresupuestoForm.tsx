...
type Item = PresupuestoFormItem;

/** Valores iniciales para el modo "editar" (se cargan del presupuesto existente). */
export interface PresupuestoFormInitial {
  cliente_id: string;
...
/**
 * Formulario de presupuesto compartido entre "Nuevo" y "Editar". En modo editar
 * conserva el mismo `numero_control` (el backend no lo toca) y hace PUT en vez de
 * POST. Recalcula ítems/descuentos/totales igual en ambos modos.
 */
export default function PresupuestoForm({
  ...
  const volverHref = esEditar && presupuestoId ? `/presupuestos/${presupuestoId}` : "/presupuestos";
  ...
  const [clienteRuc, setClienteRuc] = useState(initial?.cliente_ruc ?? "");
  const [clienteTel, setClienteTel] = useState(initial?.cliente_telefono ?? "");
  const [clienteDir, setClienteDir] = useState(initial?.cliente_direccion ?? "");

  // Items
  const [items, setItems] = useState<Item[]>(initial?.items ?? []);

  // Condiciones
  const [condicion, setCondicion] = useState<CondicionPresupuesto>(initial?.condicion ?? "contado");
  const [validezDias, setValidezDias] = useState(initial?.validez_dias ?? "15");
  const [formaPago, setFormaPago] = useState(initial?.forma_pago ?? "");
  const [plazoEntrega, setPlazoEntrega] = useState(initial?.plazo_entrega ?? "");
  ...
  async function guardar() {
    ...
    setError(null);
    try {
      const url = esEditar && presupuestoId ? `/api/presupuestos/${presupuestoId}` : "/api/presupuestos";
      ...
      const destino = esEditar && presupuestoId ? presupuestoId : body.data.id;
      router.push(`/presupuestos/${destino}`);
    } catch {
  ...
  return (
    <div className="space-y-6">
      <Link href={volverHref} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
        <ArrowLeft className="h-4 w-4" /> {esEditar ? "Volver al presupuesto" : "Volver a presupuestos"}
      </Link>

      <div className="flex items-center gap-3">
        <FileText className="h-7 w-7 text-[#4FAEB2]" />
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">
          {esEditar ? `Editar presupuesto ${numeroControl ?? ""}`.trim() : "Nuevo presupuesto"}
        </h1>
      </div>
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
        ...
        <button onClick={guardar} disabled={!valido || guardando} className="inline-flex items-center justify-center gap-1.5 rounded-md bg-[#4FAEB2] px-5 py-2 text-sm font-medium text-white hover:bg-[#3F8E91] disabled:opacity-50">
          {guardando ? <><Loader2 className="h-4 w-4 animate-spin" /> Guardando…</> : esEditar ? "Guardar cambios" : "Guardar presupuesto"}
        </button>
      </div>

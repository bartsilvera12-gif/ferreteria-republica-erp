/**
 * Límites de día y mes calendario en America/Asuncion (PY, UTC−3 fijo),
...
export function asuncionMonthBoundsUtc(now = new Date()): { start: string; end: string } {
  ...
  const start = new Date(`${y}-${String(monthNum).padStart(2, "0")}-01T00:00:00-03:00`);
  const nextY = monthNum === 12 ? y + 1 : y;
  const nextM = monthNum === 12 ? 1 : monthNum + 1;
  const end = new Date(
    `${nextY}-${String(nextM).padStart(2, "0")}-01T00:00:00-03:00`
  );

...
/** PUT /api/ordenes-compra/[numero] — edita una OC pendiente (cabecera + líneas). */
export async function PUT(
  ...
  try {
    ...
    const header: OrdenCompraHeaderInput = {
      ...
      // Comprobante: solo si se envía uno nuevo; si no, el server conserva el existente.
    ...
    try {
      const out = await actualizarOrdenCompra(schema, empresaId, numeroOc, header, items);
      return NextResponse.json(successResponse({ numero_oc: out.numero_oc, ordenes: out.ordenes }));
    } catch (e) {
      const msg = e instanceof Error ? e.message : "No se pudo editar la orden de compra.";
      const status = /no encontrada/i.test(msg) ? 404 : /no se puede editar/i.test(msg) ? 409 : 500;
      console.error("[/api/ordenes-compra/[numero] PUT]", { empresaId, numeroOc, msg });

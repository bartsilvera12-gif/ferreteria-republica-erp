...
function buildResumenInferido(
  parsed: ConsultaLoteRespuestaParsed,
  infer: { nuevoEstado: "aprobado" | "rechazado" | null; filaRelevante: { dEstRes: string; grupo_res: { dMsgRes: string }[] } | null },
  _estadoFinal: string,
 ...
 * Consulta resultado de lote en SIFEN TEST usando `sifen_d_prot_cons_lote`.
 */
export async function POST(
  ...
  try {
    ...
    const { data: feRow, error: errFe } = await supabase
      ...
      .select(
        "id, factura_id, estado_sifen, cdc, error, sifen_d_prot_cons_lote, sifen_ultima_respuesta_consulta_lote"
      )
      .eq("factura_id", fid)
    ...
    const protRaw =
      feRow.sifen_d_prot_cons_lote == null ? "" : String(feRow.sifen_d_prot_cons_lote).trim();
    if (!protRaw || !/^[0-9]+$/.test(protRaw)) {
      return NextResponse.json(
        errorResponse(
          "No hay protocolo de lote (sifen_d_prot_cons_lote). Envíe primero el lote con POST .../sifen/enviar-test."
        ),
        { status: 409 }
      );
    }
    ...
    if (String(cfg.ambiente) !== "test") {
      return NextResponse.json(
        errorResponse(
          'Este endpoint solo opera con configuración SIFEN en ambiente "test".'
        ),
        { status: 400 }
      );
    }
    ...
    if (!cfg.activo) {
      return NextResponse.json(
        errorResponse("La configuración SIFEN está inactiva."),
        { status: 400 }
      );
    }
    ...
    try {
      resp = await consultarLoteSifenTest({
      ...
      return NextResponse.json(
        errorResponse(`Fallo al llamar a SIFEN TEST (consulta-lote): ${m}`),
        { status: 502 }
      );
    }
